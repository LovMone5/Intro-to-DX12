...
private:
	...
	std::unique_ptr<SobelFilter> mBlurFilter;
...
bool BlurApp::Initialize()
{
	...
	mBlurFilter = std::make_unique<SobelFilter>(mD3DDevice.Get(), mClientWidth, mClientHeight);
...
void BlurApp::Draw(const GameTimer& gt)
{
	...
	mBlurFilter->Execute(mCommandList.Get(), CurrentBackBuffer());

	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		mSwapChainBuffer[mCurrentBackBufferIndex].Get(),
		D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));

	mCommandList->CopyResource(CurrentBackBuffer(), mBlurFilter->OutputBuffer());
...
void BlurApp::BuildDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors = 3 + 4;
	...
	mBlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(mSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 3, mCbvSrvDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 3, mCbvSrvDescriptorSize),
...
void BlurApp::BuildLandGeometry()
{
	...
	geo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(mD3DDevice.Get(),
		mCommandList.Get(), geo->VertexBufferCPU.Get(), geo->VertexBufferUploader);

	geo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(mD3DDevice.Get(),
		mCommandList.Get(), geo->IndexBufferCPU.Get(), geo->IndexBufferUploader);
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> BlurApp::BuildStaticSamplers()
{
