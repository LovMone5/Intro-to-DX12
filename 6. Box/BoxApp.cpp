...
{
	ThrowIfFailed(mMainCmdAllocator->Reset());
	ThrowIfFailed(mCommandList->Reset(mMainCmdAllocator.Get(), mPSO.Get()));
...
{
	mVertexShader = d3dUtilBox::CompileShader(L"Shaders/color.hlsl", "VS", "vs_5_0");
	mPixelShader = d3dUtilBox::CompileShader(L"Shaders/color.hlsl", "PS", "ps_5_0"); 
...
void BoxApp::BuildBoxAndPyramidGeometry()
{
	...
	mBoxGeo = std::make_unique<d3dUtilBox::MeshGeometry>();
